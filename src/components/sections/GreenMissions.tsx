  ...
  return (
    <div className={cn("rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col h-full", className)}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-slate-700">Green Missions</h3>
          ...
          >
            <div className="flex items-center gap-3">
              {mission.completed ? (
                ...
                <Circle size={20} className="text-slate-300 group-hover:text-emerald-400 transition-colors" />
              )}
              <span className={cn(
                "text-sm font-medium transition-colors",
                mission.completed ? "text-emerald-600/70 line-through" : "text-slate-700"
              )}>
                {mission.title}
              </span>
            </div>
            <span className={cn(
              ...
              mission.completed ? "bg-emerald-100 text-emerald-500/50" : "bg-emerald-100 text-emerald-600"
            )}>
              +{mission.points}
            </span>
          </motion.div>
      ...
      <div className="mt-6 pt-4 border-t border-slate-200 flex items-center justify-between">
        ...
        <span className="text-xl font-bold text-emerald-600">{totalPoints} pts</span>
      </div>
    </div>
  );
}
